...
function setDate(){
    ...
    console.log("Set current date: ", dateReadable);
}
...
// read saved planner information from localStorage
function getPlanBrowserStorage(date) {
    console.log("getPlanBrowserStorage()");
...
function renderPage(){
    ...
    var plans = getPlanBrowserStorage(date);
    var workDayHour = moment(dateIndex+"T09");
    console.log(workDayHour);

    $(".set-now").text(dateReadable);

    // determine if time slot is in past present or future, set CSS class
    var relTime=""; // past, present, future
